...
if uploaded_file is not None:
    try:
        ...
        if not all(col in df.columns for col in required_cols) or '강성' not in df.columns:
            ...
            st.write("🕵️‍♂️ **현재 파일에서 인식된 컬럼 목록:**", list(df.columns))
        ...
        st.error(f"⚠️ 시스템 연산 실행 중 예상치 못한 오류가 발생했습니다: {str(e)}")
else:
    # 최초 진입 시 가이드 문구 출력
